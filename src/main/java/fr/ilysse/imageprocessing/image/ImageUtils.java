...
//    }
    ...
    public static Map<String,Integer> getXYBoundsWithLAB(BufferedImage bufferedImage){
        ...
        clusterUtils.executeKMeans(3,3,datas.getListOfLAB(),false);
...
//        displayImage(getClusteredImage(bufferedImage,datas.getLABPMapToRGB()));
    ...
    public static BufferedImage getComputedImage(BufferedImage bufferedImage,int xTemplate,int yTemplate,int yMargin,int outputWidth,int outputHeight){
        ...
        int maxDimImage = bufferedImage.getWidth() >= bufferedImage.getHeight() ? bufferedImage.getWidth() : bufferedImage.getHeight();
        int maxDim;
        if(maxDimTemplate==xTemplate && maxDimImage == bufferedImage.getWidth()){
            maxDim=xTemplate;
        }else if(maxDimTemplate==yTemplate && maxDimImage==bufferedImage.getHeight()){
            maxDim=yTemplate;
        }else if(maxDimTemplate==xTemplate && maxDimImage==bufferedImage.getHeight()){
            maxDim=yTemplate;
        }else if(maxDimTemplate==yTemplate && maxDimImage==bufferedImage.getWidth()){
            maxDim=xTemplate;
        }else{
        ...
        Map<String,Integer> initialImageDatas = getXYBounds(getBWFromRGB(bufferedImage));
        ...
        BufferedImage cuttedImage = cutImageFromCenter(resizedImage,xTemplate,yTemplate);
        BufferedImage finalImage = getFinalImageFromCenterImage(outputWidth,outputHeight,yMargin,cuttedImage);
        return finalImage;
    }
